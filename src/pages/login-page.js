...
const LoginPage = () => {
    ...
    const handleLogin = async(e) => {
      ...
      try {
        ...
        const respUser = await axios.get(`${API_BASE_URL}/user`,{headers:authHeader})
    ...
    <Container>
      <Form onSubmit={handleLogin}>
        <Form.Group className="mb-3" controlId="formBasicEmail">
          ...
          <Form.Control type="email" value={email} onChange={(e)=>setEmail(e.target.value)} required={true}/>
        </Form.Group>
        ...
        <Button variant="primary" type="submit" disabled={loading} >{loading && <Spinner animation="border" variant="danger" size="sm"/>
 }Login</Button>
      </Form>
    </Container>
  );
